...
data1 = filter daysbetween_data by catag  == 'L' ;
 ...
 grouped = group data1 by (user_location_country,is_mobile);
 counted = FOREACH grouped GENERATE group as is_mobile, COUNT(data1) as cnt;
 result = FOREACH counted GENERATE is_mobile.user_location_country,is_mobile.is_mobile,cnt;
